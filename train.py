...
def train_epoch(args, do_training, model, device, train_loader, test_loader, val_loader, optimizer, benchType,
    ...
    for param in model.parameters():
        if param is not model.classifier.weight and param is not model.classifier.bias:
    ...
    for batch_idx, (seq_lengths, data, label) in pbar:
        ...
        optimizer.zero_grad()
        softmax_output, output = model(args, data, seq_lengths, onehot_labels_yt, do_training, mask_model)  # def forward(self, x, yt, do_training):
