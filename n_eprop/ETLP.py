...
parser = argparse.ArgumentParser(
    description="Code Thief"
)

# Learning parameters
parser.add_argument("--seed", type=int, default=42, help="The truth of the Universe")
...
parser.add_argument("--recurrent", action="store_true", help="Recurrent Connection")
...
def main():
    ...
    loss_fn = lambda x, y: 0.5 * torch.sum((x - y) ** 2)  # same as F.mse_loss
    ...
    for epoch in range(args.epochs):
        ...
        (
            ...
            recurrent_v_train,
            output_v_train,
        ) = plot_images(input_train, recurrent_v_train, recurrent_s_train, output_v_train, output_train)
        #   test_plot_images
        (
            input_test,
            recurrent_s_test,
            ...
            recurrent_v_test,
            output_v_test,
        ) = plot_images(input_test, recurrent_v_test, recurrent_s_test, output_v_test, output_test)
        ...
        if acc_test > best_acc:
            best_acc = acc_test
            torch.save(model.state_dict(), f"best_model_{best_acc}.pth")
            print("Saving model with Test Accuracy: {:.2f}%".format(best_acc))
